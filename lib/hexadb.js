...
HexaDB.prototype = {
  ...
  searchStream: function( query, options ) {
    ...
    // Orders queries by approximate size
    // (smallest first)
    ...
    // Create a stream for each query
    // var streams = queries.map( function( query ) {
    //   return self.getStream( query )
    // })
    
    var solution = new HexaDB.Solution( options )
    
    async.eachSeries( queries, function iterator( query, next ) {
      
      var stream = self.getStream( query )
    ...
    // TODO:
    // - [ ] alter HDB.Query() to handle variables
    // - [x] map triple patterns (query elements) to queries
    // - [ ] order queries by approximate size (smallest first)
    // - [x] create a (get?)Stream for each query
    // - [x] reassemble stream results
    // - [ ] map results to queried variables
    // - [ ] stream out results
